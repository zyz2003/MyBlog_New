...
| 2.0   | 核心框架        | 插件系统、主题系统、钩子系统    | packages/core     |
| 3.0   | 数据库层        | Schema 定义、ORM 配置、迁移脚本 | packages/database |
| 4.0   | API 层          | Nitro 路由、中间件、服务层      | apps/site/server  |
...
| 技术          | 用途            | 备注           |
| ------------- | --------------- | -------------- |
| Tailwind CSS  | 原子化 CSS      | 主要样式方案   |
| CSS Variables | 动态主题        | 主题切换核心   |
| UnoCSS        | (可选) 按需生成 | 性能优化时使用 |
...
| 技术   | 用途             | 位置         |
| ------ | ---------------- | ------------ |
| TipTap | 可视化富文本编辑 | 后台文章编辑 |
| Vditor | Markdown 编辑    | 后台文章编辑 |
...
| 数据库 Schema | `packages/database/src/schema/` | `schema/posts.ts`            |
